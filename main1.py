...
if __name__ == "__main__":
    ...
    class Args:
        ...
        batch_size = 64                    # kích thước batch cho local training
        ...
        save_root = "./results1000"           # thư mục gốc lưu kết quả
    ...
    for name, cls in datasets_cfg:
        ...
        for part_name, fn in partitions.items():
            print(f"\n--- Partition: {part_name} ---")
            # 7.2) Sinh các chỉ số (index lists) cho client
            idxs = fn(train_ds)
            # 7.3) Đảm bảo mỗi client có ít nhất 1 mẫu
